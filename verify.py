...
def verify_topq(dir, lr, batch_size, dataset, architecture, save_freq, order, threshold, epochs=1, q=10, half=0):
    ...
    for epoch in range(1): # for i in range(epochs):
...
def verify_training_main():
    architecture = eval(f"custom_model.resnet20")
    res = verify_topq('proof/CIFAR10_test', 0.01, 128, 'CIFAR10', architecture, 1000, ['1', '2', 'inf', 'cos'], [1000, 10, 0.1, 0.01], 2, 2)
    return res
